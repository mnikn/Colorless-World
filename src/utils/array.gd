...
static func flat(arr: Array) -> Array:
	var res = []
	for item in arr:
		if item is Array:
			res.append_array(ArrayUtils.flat(item))
		else:
			res.push_back(item)
	return res
...
static func reduce(arr: Array, initial_value: Variant, fn: Callable) -> Variant:
	var result = initial_value
	for element in arr:
		result = fn.call(result, element)
	return result
