...
static func _do_get_value(obj, property_arr: Array, default_value):
	if len(property_arr) <= 0 or obj == null:
		return default_value
	var tmp = null
	if obj is Dictionary and obj.has(property_arr[0]):
		tmp =  obj[property_arr[0]]
	if obj is Object and ArrayUtils.find(obj.get_property_list(), func (item): return item.name == property_arr[0]) != null:
		tmp =  obj[property_arr[0]]
