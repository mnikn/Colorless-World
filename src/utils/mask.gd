...
static func show_mask(target_node: Node, duration = 0.2, color = Color("#939393"), control_wrap = false):
	if target_node.has_node("Modulate"):
		await target_node.get_tree().create_timer(0.0).timeout
#		return target_node.get_node("Modulate")
		return
	...
	if control_wrap:
		...
		control.name = "Modulate"
#		RenderingServer.canvas_item_set_z_index(control.get_canvas_item(), 128)
	...
	if duration <= 0:
		...
		tween.tween_property(node, "color", color, duration)
...
#	var mask_node = parent_node.find_node("Modulate", true, false)
	...
	if mask_node != null and not mask_node.is_queued_for_deletion() and mask_node.get_tree() != null:
		var tween = mask_node.get_tree().create_tween()
		if mask_node is Control:
			tween.tween_property(mask_node.get_children()[0], "color", Color("#ffffff"), duration)
		else:
